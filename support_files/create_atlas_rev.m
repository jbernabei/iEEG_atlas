function [median_conn, std_conn, num_samples, sem_conn, raw_atlas] = create_atlas_rev(all_conn, all_roi, all_resect, region_list, threshold)
...
num_roi = length(region_list);

% initialize output array to store connection strengths
median_conn = NaN(num_roi,num_roi,num_patients);
...
for p = 1:num_patients
    ...
    band_matrix = all_conn{p};
    

    % double loop through regions
    for i = 1:num_roi % first region
...
%         end
        ...
        for j = 1:num_roi % second region (used to be (i+1):90)
